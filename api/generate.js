...
사용할 수 있는 질문 어미는 "옳은 것은?", "옳지 않은 것은?", "가장 적절한 것은?", "가장 적절하지 않은 것은?", "해당하지 않는 것은?", "가장 정확한 것은?" 등이며, 한 번에 만드는 2~3문제 안에서 같은 질문형과 같은 문장 구조를 반복하지 말고 서로 다르게 섞어라. 다음과 같은 문제 구조도 적극 활용해 다양성을 높여라(모두 제공된 조문에 실제로 있는 내용만 사용).
- 조문이 여러 항·호·목으로 요건이나 예외를 나열하고 있다면, 그 나열된 항목들을 선택지로 쓰고 그중 하나만 조문에 없는 그럴듯한 항목으로 바꿔서 "다음 중 OOO에 해당하지 않는 것은?" 형태로 묻는다.
- 조문이 정한 의무나 절차를 지키는 상황과 지키지 않는 극단적인 상황을 대비시켜 "OOO 상황에서 개인정보처리자가 취해야 할 조치로 가장 적절한 것은?" 형태로 묻는다.
- 조문에 예외나 제한 요건이 있다면 "~할 수 없는 경우는?" 또는 "~할 수 있는 경우로 가장 적절한 것은?"처럼 예외 해당 여부를 판단하게 한다.
